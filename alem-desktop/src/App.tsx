...
const css = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .auth-container {
    width: 400px; padding: 40px; background: rgba(255,255,255,0.02);
...
export default function App() {
  ...
  const [tab, setTab] = useState<"docs" | "create" | "identity">("docs");
  ...
  const loadCaptcha = async () => {
    try { 
      ...
      setCaptcha({ token: res.token, answer: "" }); 
  ...
  const handleLogin = async () => {
    ...
    try {
      const result = await invoke<any>("login_user", { ...loginForm });
  ...
  const handleRegister = async () => {
    ...
    try {
      const result = await invoke<any>("register_user", { 
        nickname: regForm.username,
        email: regForm.email,
        password: regForm.password,
        dateOfBirth: regForm.dateOfBirth,
        captchaToken: captcha?.token, 
        captchaAnswer: regForm.code 
      });
  ...
  const handleVerifyEmail = async () => {
    ...
    setLoading(true);
    try {
      const result = await invoke<any>("verify_email", { userId: regUserId, code: regForm.code });
      setDid(result.did);
      setView("dashboard");
      loadDocs();
  ...
  const startRecording = async () => {
    try {
      ...
      addToast("🎙️ Recording...", "info");
    } catch (e) { addToast("Mic access denied", "error"); }
  };
            ...
            regStep === 2 ? (
              <div>
                ...
                <div className="input-group">
                  <input className="input" placeholder="Desired Username" value={regForm.username} onChange={e => setRegForm({...regForm, username: e.target.value})} />
                </div>
                <div className="input-group">
                  <input className="input" placeholder="Email Address" value={regForm.email} onChange={e => setRegForm({...regForm, email: e.target.value})} />
                </div>
                ...
                {captcha && (
                  <div className="captcha-box">
                    <div style={{ background: '#fff', padding: '12px', borderRadius: 12, marginBottom: 16, display: 'inline-block' }} dangerouslySetInnerHTML={{ __html: captcha.token }} />
                    <input className="input" placeholder="Enter characters above" value={regForm.code} onChange={e => setRegForm({...regForm, code: e.target.value})} />
                  </div>
                )}
    ...
    <>
      ...
      <div className="app">
        <header className="header">
          ...
          <div className="header-right">
            <div>
              ...
              {isOnline ? "VAULT CONNECTED" : "OFFLINE"}
            </div>
            <div style={{ maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: 'JetBrains Mono', fontSize: 10, opacity: 0.5 }}>{did}</div>
        ...
        <nav className="sidebar">
          ...
          <div className={`nav-item ${tab === 'identity' ? 'active' : ''}`} onClick={() => setTab('identity')}>
            <span>⬢ Identity</span>
          </div>
        </nav>
        ...
        <main className="main">
              ...
              <div className="doc-list">
                {displayDocs.map(d => (
                  <div key={d.id} className="card">
                    ...
                    <div className="doc-meta">{d.content_type?.split('/')[1]?.toUpperCase() || 'BIN'} · {new Date(d.updated_at).toLocaleDateString()}</div>
                    <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                       <button className="btn" style={{ flex: 1, fontSize: 11 }}>VIEW</button>
                       <button className="btn" style={{ flex: 1, fontSize: 11, color: '#4A9EFF', borderColor: 'rgba(74,158,255,0.2)' }} onClick={async () => {
                          const { save } = await import("@tauri-apps/plugin-dialog");
                          const { writeFile } = await import("@tauri-apps/plugin-fs");
                          const path = await save({ defaultPath: d.filename });
                          if (path) {
                            addToast("Downloading...", "info");
                            const bytes = await invoke<number[]>("get_file_bytes", { docId: d.id });
          ...
          {tab === 'identity' && (
            <div className="panel">
              <div style={{ fontSize: 32, fontWeight: 800 }}>Secure Identity</div>
              <div style={{ opacity: 0.3, marginBottom: 40 }}>Your cryptographically unique identifier</div>
              <div className="did-box">{did}</div>
              <div style={{ fontSize: 12, color: '#00E0C6' }}>● ACTIVE</div>
            </div>
          )}
        </main>
        ...
        <div 
          style={{
            ...
            borderRadius: '50%', background: recState === 'recording' ? '#FF6B6B' : '#4A9EFF',
            display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
            boxShadow: '0 8px 32px rgba(0,0,0,0.3)', transition: 'all 0.3s'
          }}
          onClick={() => recState === 'recording' ? stopRecording() : startRecording()}
        >
          {recState === 'recording' ? <span>{recDuration}s</span> : <span style={{fontSize: 28}}>🎙️</span>}
        </div>
      </div>
      <div className="toast-container">{toasts.map(t => <div key={t.id} className={`toast`}>{t.msg}</div>)}</div>
    </>
