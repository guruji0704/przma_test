...
def reset_password(conn, params) do
  ...
  # GET /reset-password
  # ===========================================================================
    def reset_password_page(conn, params) do
      user_id = params["user_id"]
      token = params["token"]

      # Render a simple HTML page for the user
      html(conn, """
      <!DOCTYPE html>
      <html>
      <head>
        <title>Reset Password - ALEM</title>
        <style>
          ...
          h2 { color: #333; }
          p { color: #666; }
          .code { background: #eee; padding: 10px; border-radius: 4px; font-family: monospace; margin: 20px 0; word-break: break-all; }
      ...
      <body>
        <div class="card">
          ...
          <p>Your reset token is ready. Please return to the ALEM app to finish.</p>
          ...
          <p style="font-size: 12px; color: #999;">Copy the User ID and Token above into the ALEM App.</p>
        </div>
      </body>
      </html>
      """)
