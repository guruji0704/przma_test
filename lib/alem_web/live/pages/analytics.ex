defmodule AlemWeb.Admin.Pages.Analytics do
  @moduledoc "Drill-down analytics: Users, Storage, CAS. Add new analytics pages here."
  ...
  def users(%{analytics_users: nil} = assigns), do: ~H"""
  <div>
  ...
  def users(assigns) do
    ...
    pie_chart = Charts.pie(["Verified","Unverified"], [a.verified, a.unverified], ["#3fb950","#484f58"])
  ...
  def storage(%{analytics_storage: nil} = assigns), do: ~H"""
  <div>
    <.back_button nav_history={@nav_history} />
  ...
  def storage(assigns) do
    ...
    pie_chart = Charts.pie(Enum.map(top8, & short_mime(&1.type)), Enum.map(top8, & &1.count), colors)
  ...
  def cas(%{analytics_cas: nil} = assigns), do: ~H"""
  <div>
    <.back_button nav_history={@nav_history} />
  ...
  def cas(assigns) do
    ...
    donut_chart = Charts.doughnut(["Unique","Duplicates"], [a.total_cas - a.dupes, a.dupes], ["#3fb950","#e3b341"])

    waste   = a.saved_bytes
    actual  = max(a.total_bytes - waste, 0)
    pie_chart = Charts.pie(["Actual Storage","Wasted (dupes)"], [actual, waste], ["#58a6ff","#f85149"])
