defmodule Alem.Cas.CasObject do
  @moduledoc """
  One row = one unique file, identified by its SHA-256 content hash.
  Two users uploading the same bytes → same hash → same row → ONE S3 object.
  ref_count tracks how many users/documents point to these bytes.
  S3 bytes deleted ONLY when ref_count reaches 0.
  """
  use Ecto.Schema
  ...
  schema "cas_objects" do
    ...
    # Content metadata (extracted on ingest)
